      ...
      <CardContent className="p-4 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <Checkbox
            ...
            className="h-6 w-6 data-[state=checked]:bg-accent data-[state=checked]:border-accent focus-visible:ring-accent"
          ...
          <div className="flex-1 min-w-0">
            <label htmlFor={`paid-${bill.id}`} className={cn("block font-semibold text-lg truncate", bill.paid && "line-through text-muted-foreground")}>
              {bill.name}
            </label>
            <p className={cn("text-sm", bill.paid ? "text-muted-foreground" : "text-foreground/80")}>
              Due: {format(formattedDueDate, 'MMMM d, yyyy')}
            </p>
          </div>
        </div>
