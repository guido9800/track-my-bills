...
export default function SignupPage() {
  ...
  const handleEmailPasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Welcome! You've successfully signed up with email.",
      });
      router.push('/');
    } catch (err: any) {
  ...
  // Temporarily commenting out social signup functionality
  ...
  //   } else {
  //     setError("Invalid signup provider.");
  //     setLoading(false);
  //     return;
  //   }
  ...
  //     toast({
  //       title: "Account Created",
  //       description: `Welcome! You've signed up with ${providerName.charAt(0).toUpperCase() + providerName.slice(1)}.`,
  //     });
  //     router.push('/');
  //   } catch (err: any) {
  ...
  //        errorMessage = "Sign-up process was cancelled. Only one sign-up attempt is allowed at a time.";
  //     } else if (err.code === 'auth/popup-blocked') {
  //       errorMessage = "Sign-up popup was blocked by the browser. Please disable your popup blocker and try again.";
  //     } else if (err.code === 'auth/unauthorized-domain') {
  //       errorMessage = "This domain is not authorized for this sign-up method. Please check Firebase configuration.";
  //     } else {
  //       errorMessage = err.message || errorMessage;
  //     }
  //     setError(errorMessage);
    ...
    <div className="container mx-auto flex min-h-[calc(100vh-4rem)] items-center justify-center p-4">
      <Card className="w-full max-w-md shadow-2xl">
        <CardHeader className="text-center">
          ...
          <CardDescription>Join Track-My-Bills to manage your expenses.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <form onSubmit={handleEmailPasswordSubmit} className="space-y-6">
            ...
            <Button type="submit" className="w-full text-base" disabled={loading}>
              {loading && !email && !password ? 'Signing up...' : loading ? 'Creating Account...' : 'Sign Up with Email'}
            </Button>
          </form>
          ...
          {/* Temporarily hiding social signup options */}
          {/*
